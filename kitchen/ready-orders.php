...
if (isset($_GET['served']) && is_numeric($_GET['served'])) {
    $order_id = (int)$_GET['served'];
    try {
        $stmt = $db->prepare("UPDATE food_orders SET status = 'completed', updated_at = NOW() WHERE id = ? AND branch_id = ? AND status = 'ready'");
        $stmt->execute([$order_id, $branch_id]);
        if ($stmt->rowCount()) {
            set_flash('success', 'Order marked as served/completed.');
        }
    } catch (Exception $e) {
        error_log('Served order error: ' . $e->getMessage());
...
<div class="container-fluid">
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Ready Orders</li>
        </ol>
    </nav>

    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="mb-0 fw-semibold">Orders Ready for Serving</h4>
        <span class="badge bg-success fs-6">
            <?php
            try {
                $stmt = $db->prepare("SELECT COUNT(*) FROM food_orders WHERE branch_id = ? AND status = 'ready'");
                $stmt->execute([$branch_id]);
                echo $stmt->fetchColumn() . ' ready';
            } catch (Exception $e) {
                echo '0 ready';
            }
            ?>
    ...
    try {
        $stmt = $db->prepare("
            SELECT fo.*, u.full_name as waiter_name,
                   EXTRACT(EPOCH FROM (NOW() - fo.updated_at)) / 60 as ready_minutes
            FROM food_orders fo 
            LEFT JOIN users u ON fo.waiter_id = u.id 
            WHERE fo.branch_id = ? AND fo.status = 'ready' 
            ORDER BY fo.updated_at ASC
        ");
        $stmt->execute([$branch_id]);
    ...
    ?>
        <div class="text-center py-5">
            ...
            <p class="text-muted mt-2 mb-0">No ready orders. All orders are being prepared or completed.</p>
        </div>
    <?php else: ?>
        <div class="row g-3">
            <?php foreach ($orders as $order):
                ...
                $ready_mins = $order['ready_minutes'];
                $ready_display = $ready_mins < 60 ? $ready_mins . ' min' : floor($ready_mins / 60) . ' hr ' . ($ready_mins % 60) . ' min';
                $ready_class = $ready_mins > 10 ? 'danger' : ($ready_mins > 5 ? 'warning' : 'success');
            ?>
            <div class="col-12 col-md-6 col-lg-4">
                <div class="card border-0 shadow-sm h-100 order-card ready">
                    <div class="card-header bg-success bg-opacity-10 border-bottom-0 py-3 d-flex justify-content-between align-items-center">
                        <div>
                            <h6 class="mb-0 fw-bold"><?php echo htmlspecialchars($order['order_reference']); ?></h6>
                            <small class="text-<?php echo $ready_class; ?>">
                                <i class="bi bi-clock"></i> Ready <strong><?php echo $ready_display; ?></strong> ago
                            </small>
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center border-top pt-3">
                            ...
                            <a href="?served=<?php echo $order['id']; ?>" class="btn btn-success btn-sm">
                                <i class="bi bi-hand-thumbs-up"></i> Served / Collected
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <?php } catch (Exception $e) {
        ...
        echo '<div class="alert alert-danger">Unable to load ready orders.</div>';
    } ?>
</div>
